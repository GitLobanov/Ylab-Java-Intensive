...
public class OrderMenuHandler extends BaseHandler{
    ...
    public void create() {
        ...
        Order order = new Order(0, car, user, Order.TypeOrder.BUYING, note);
    ...
    public void update() {
        ...
        while (order == null) {
            ...
            try {
                ...
                ErrorResponses.printCustomMessage("Hey what's wrong with ID, I cannot find order");
    ...
    @Override
    public void search() {
        String query = orderService.formingQuerySearchOrders(Order.TypeOrder.BUYING);
        orderService.getOrdersBySearch(query);
    }

    public void getByClient(){
        printList(orderService.findOrderByClientAndTypeOrder(Session.getInstance().getUser(), Order.TypeOrder.BUYING));
    }
    ...
    @Override
    public void viewAll() {
        printList(orderService.getAllBuyingOrders());
    }

    public void takeOrderForManager() {
        System.out.println("\uD83C\uDD99 Taking order");
        System.out.println("Input please, id of order you want to take: ");
        String inputDeleteId = scanner.nextLine().trim();
        Order orderForTake = orderService.findOrderById(Integer.parseInt(inputDeleteId));
        ...
        orderService.updateOrder(orderForTake);
    }
}
